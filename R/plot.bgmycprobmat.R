...
function(x, tree, ...) {
  ...
  image(x = c(1:ntip + 1), y = c(1:ntip), z = orderedmat, axes = FALSE, 
        breaks = c(0, 0.05, 0.49999999, 0.899999999, 0.9499999999, 1), col = heat.colors(5))
  
  par(fig = c(0.968, 0.99, 0.035, 0.965), new = TRUE)
  leg <- matrix(nrow = 1, ncol = 20, data = seq(from = 0.025, to = 1, by = 0.05))
  image(y = seq(from = 0, to = 1, by = 0.05), x = 1, z = leg, col = heat.colors(5), 
        breaks = c(0, 0.05, 0.5, 0.9, 0.95, 1), axes = FALSE)
